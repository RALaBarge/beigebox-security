...
# Auto-detects your system, asks simple questions, sets up everything
# Safe to re-run. After first run, ./launch.sh just works.
#
# USAGE:
#   ./FIRST_RUN.sh           # Normal setup (interactive, preserves existing config)
#   ./FIRST_RUN.sh --reset   # Force reconfigure (overwrites ~/.beigebox/config)
...
if [[ -f "$CONFIG_FILE" && "$ALLOW_RESET" != "--reset" ]]; then
    ...
    echo "To reconfigure everything, run: ./FIRST_RUN.sh --reset"
    echo ""
...
# To reconfigure: ./FIRST_RUN.sh --reset
