...
<style>
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  /* ── Mobile breakpoint: < 768px (major layout changes) ───────────────────── */
  @media (max-width: 767px) {
    ...
    /* Show hamburger menu, hide status bar on mobile */
    #menu-toggle {
      display: block;
    }
    ...
    /* Hide tab key indicator on mobile (saves space) */
    .tab .tab-key {
      display: none;
    }
    ...
    /* ── KV rows: more flexible wrapping ──────────────────────────────────── */
    .kv-row {
      flex-direction: column;
      gap: 4px;
      padding: 6px 0;
    ...
    /* ── Input fields & buttons: touch-friendly (44px minimum) ────────────── */
    ...
    /* ── Config panel: single column input ────────────────────────────────── */
    .cfg-group {
      margin: 8px 0;
    }
    .cfg-input-row {
    ...
    /* ── Tap filter controls: full width ──────────────────────────────────── */
    .tap-filter-row {
      flex-direction: column;
      gap: 6px;
    }
    .tap-filter-row label {
      font-size: 11px;
    }
    .tap-filter-row input,
    .tap-filter-row select {
      width: 100%;
      min-height: 40px;
      font-size: 14px;
    }
...
<div id="tabs">
  ...
  <div class="tab" data-tab="harness"   onclick="switchTab('harness')">  <span class="tab-key">7</span>Harness</div>
  <div class="tab" data-tab="config"    onclick="switchTab('config')">   <span class="tab-key">8</span>Config</div>
</div>
...
<div id="content">
  ...
  <div class="panel" id="panel-chat">
    <div class="toolbar">
      ...
      <button onclick="startEnsembleVote()" title="Vote on responses from multiple models" style="margin-right:8px">🎯 Ensemble</button>
  ...
  <div class="panel" id="panel-operator">
    <div class="toolbar">
      ...
      <span class="toolbar-spacer"></span>
      <span style="color:var(--fg-muted);font-size:10px">LangChain ReAct · tool registry</span>
      <span class="toolbar-spacer"></span>
  ...
  <div class="panel" id="panel-harness">
    ...
    <div class="toolbar">
      ...
      <span id="harness-manual-controls" style="display:flex;gap:6px;align-items:center">
        ...
        <select id="harness-add-select" style="height:26px;font-size:12px;padding:2px 6px;min-width:160px">
          <option value="">— pick agent/model —</option>
          <option value="operator">operator</option>
        </select>
        <button onclick="harnessAddTarget()">＋ Add</button>
        ...
        <div id="harness-page-nav" style="border:none;padding:0">
          <button onclick="harnessPagePrev()" id="harness-prev-btn" title="Previous page">◀</button>
          <span id="harness-page-indicator">p1/1</span>
          <button onclick="harnessPageNext()" id="harness-next-btn" title="Next page">▶</button>
        </div>
        <button onclick="harnessRun()" class="primary" id="harness-run-btn">▶ Run All</button>
        <button onclick="harnessClear()">✕ Clear</button>
      </span>
    ...
    <div style="padding:6px 12px;background:var(--bg-panel);border-bottom:1px solid var(--bg-border);display:flex;gap:8px;flex-shrink:0">
      <textarea id="harness-prompt" rows="2"
        ...
        onkeydown="if(event.ctrlKey&&event.key==='Enter'){harnessMode==='manual'?harnessRun():harnessOrchRun()}"
...
function switchTab(name) {
  ...
  closeMobileMenu();  // Close menu on tab select
...
document.addEventListener('keydown', e => {
  ...
  const map = {'1':'dashboard','2':'chat','3':'convos','4':'flight','5':'tap','6':'operator','7':'harness','8':'config'};
...
function featureDisabledMsg(featureName, configKey, extraHint) {
  return `<div style="background:var(--bg-panel);border:1px solid var(--bg-border);border-left:3px solid var(--yellow);
    ...
    <div style="color:var(--fg-dim);font-size:11px;line-height:1.7">
      Enable it in the <a href="#" onclick="switchTab('config');return false" style="color:var(--cyan)">Config tab (8)</a>
...
async function loadModelsForPanes() {
  ...
  if (hSel) {
    hSel.innerHTML = `<option value="">— pick agent/model —</option><option value="operator">operator</option>`;
    models.forEach(id => {
      const opt = document.createElement('option');
...
async function startEnsembleVote() {
  // Get available models from current panes or ask user
  const models = [];
  chatPanes.forEach(p => {
    if (p.target && p.target !== '@op' && !models.includes(p.target)) {
      models.push(p.target);
    }
  });

  if (models.length < 2) {
    alert('Need at least 2 different models in chat panes. Add models with different selections.');
    return;
  }

  // Get prompt from input or clipboard
  const input = document.getElementById('chat-input');
  const prompt = input.value.trim() || (await navigator.clipboard.readText().catch(() => ''));

  if (!prompt) {
    alert('Enter a prompt to ensemble vote on.');
    return;
  }

  // Get judge model (default to first model)
  const judgeModel = prompt('Judge model (blank = use first model):', models[0]);
  ...
  // Clear existing panes and add fresh ones for ensemble results
  chatPanes = [];
  chatPageIdx = 0;
  const container = document.getElementById('chat-pane-container');
  container.innerHTML = '';

  // Add judge pane at top
  const judgePane = { id: Date.now(), target: `judge:${judge}`, history: [] };
  chatPanes.push(judgePane);
  createChatPaneHtml(judgePane, container);

  // Add model panes
  for (const model of models) {
    const pane = { id: Date.now() + Math.random(), target: model, history: [] };
    chatPanes.push(pane);
    createChatPaneHtml(pane, container);
  }
  ...
  try {
    const resp = await fetch(BASE + '/api/v1/ensemble', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        prompt: prompt,
    ...
    while (true) {
      ...
      for (const line of chunk.split('\n')) {
        ...
        try {
          ...
          handleEnsembleEvent(event, models, judge);
        } catch (e) {
    ...
    alert(`Ensemble vote failed: ${e.message}`);
  } finally {
    sendBtn.disabled = false;
  }
}

function handleEnsembleEvent(event, models, judgeModel) {
  const type = event.type;

  if (type === 'dispatch') {
    const judgePaneId = chatPanes[0]?.id;
    if (judgePaneId) {
      appendMsgToPane(judgePaneId, 'system', `Querying ${event.model_count} models…`, '', 
        document.getElementById(`pane-msgs-${judgePaneId}`));
    }
  } else if (type === 'result') {
    ...
    const pane = chatPanes.find(p => p.target === model);
    if (pane) {
      const area = document.getElementById(`pane-msgs-${pane.id}`);
      appendMsgToPane(pane.id, 'assistant', response, model, area);
      // Add latency badge
      const latencyEl = document.createElement('div');
      latencyEl.style.cssText = 'font-size:10px;color:var(--fg-muted);margin-top:4px';
      latencyEl.textContent = `⏱ ${latency}ms`;
      area.appendChild(latencyEl);
    }
  } else if (type === 'evaluate') {
    const judge = event.winner;
    const reasoning = event.reasoning;
    
    const judgePaneId = chatPanes[0]?.id;
    if (judgePaneId) {
      const area = document.getElementById(`pane-msgs-${judgePaneId}`);
      appendMsgToPane(judgePaneId, 'assistant', 
        `🏆 Winner: ${judge}\n\n${reasoning}`, 'judge', area);
    }

    // Highlight winner pane
    chatPanes.forEach(p => {
      if (p.target === judge) {
        const paneEl = document.getElementById(`chat-pane-${p.id}`);
        if (paneEl) {
          paneEl.style.borderColor = 'var(--green)';
          paneEl.style.borderWidth = '2px';
        }
      }
    });
  } else if (type === 'finish') {
    console.log('Ensemble vote complete:', event);
  } else if (type === 'error') {
    const judgePaneId = chatPanes[0]?.id;
    if (judgePaneId) {
      appendMsgToPane(judgePaneId, 'error', event.message, '', 
        document.getElementById(`pane-msgs-${judgePaneId}`));
    }
  }
}

function createChatPaneHtml(pane, container) {
  const div = document.createElement('div');
  div.className = 'chat-pane';
  div.id = `chat-pane-${pane.id}`;
  div.innerHTML = `
    <div class="chat-pane-header">
      <span class="pane-label">${pane.target}</span>
      <button onclick="removeChatPane('${pane.id}')" style="margin-left:auto">✕</button>
    </div>
    <div class="chat-pane-messages" id="pane-msgs-${pane.id}"></div>`;
  container.appendChild(div);
}
...
async function loadFlight() {
  ...
  try {
    ...
    if (!d.enabled) {
      el.innerHTML = featureDisabledMsg('Flight Recorder', 'flight_recorder_enabled',
        'Records the full timing lifecycle of each request — latency per stage, routing decisions, model selection.');
      return;
    }
    ...
    el.innerHTML = apiErrorMsg(e, 'Flight Recorder', 'flight_recorder_enabled',
      'Records the full timing lifecycle of each request.');
  }
}
...
async function toggleFlight(id) {
  ...
  if (expanding && body.querySelector('.spinner')) {
    try {
      ...
      body.innerHTML = apiErrorMsg(e, 'Flight Record Detail', 'flight_recorder_enabled', '');
    }
  }
}
...
async function runOp() {
  ...
  log.insertAdjacentHTML('beforeend',
    `<div class="op-query">▶ [${esc(targetLabel)}] ${esc(query)}</div>
     <div class="op-pending" id="op-pending">⟳ thinking…</div>`);
...
function renderHarnessTargetChips() {
  ...
  el.innerHTML = harnessTargets.map(t => `
    <div class="harness-target-chip">
      ...
      <span class="remove-chip" onclick="harnessRemoveTarget('${t.key.replace(/'/g,"\\'")}')">✕</span>
    </div>`).join('');
}
...
function renderHarnessGrid() {
  ...
  visible.forEach(t => {
    ...
    const statusHtml = status ? `<span class="harness-status ${status}">${status === 'pending' ? '⟳ running…' : status}</span>` : '';
    ...
    pane.innerHTML = `
      ...
      <div class="harness-pane-output${status==='pending'?' thinking':''}">${esc(text) || '<span style="color:var(--fg-muted)">waiting…</span>'}</div>`;
...
async function harnessRun() {
  ...
  // Fire all in parallel — each updates its own pane as it completes
  ...
  const updatePane = (text, status) => {
    ...
    if (pane) {
      ...
      if (out) {
        ...
        out.textContent = text || (status === 'pending' ? 'running…' : '');
      }
      if (hdr) {
        ...
        if (status) {
          ...
          sp.textContent = status === 'pending' ? '⟳ running…' : status;
          hdr.appendChild(sp);
        }
      }
    }
...
function setHarnessMode(mode) {
  ...
  if (manBtn) { manBtn.style.background = isOrch ? 'var(--bg-input)' : 'var(--lavender)'; manBtn.style.color = isOrch ? 'var(--fg-dim)' : '#000'; }
  ...
  if (manControls) manControls.style.display = isOrch ? 'none' : 'flex';
  if (orchControls) orchControls.style.display = isOrch ? 'flex' : 'none';

  // Show/hide target areas
  ...
  if (manTargets) manTargets.style.display = isOrch ? 'none' : '';
  ...
  if (manGrid) manGrid.style.display = isOrch ? 'none' : '';
  if (orchLayout) orchLayout.style.display = isOrch ? 'flex' : 'none';

  // Update prompt placeholder
  const ta = document.getElementById('harness-prompt');
  if (ta) ta.placeholder = isOrch
    ? 'Describe the goal… the orchestrator will plan, delegate, and iterate. (Ctrl+Enter to run)'
    : 'Prompt to send to all targets… (Ctrl+Enter to run)';

  // Populate orch target chips if switching to orchestrated
  if (isOrch && harnessOrchTargets.length === 0) {
    // Default: add operator + any existing manual targets
...
function renderOrchTargetChips() {
  ...
  el.innerHTML = harnessOrchTargets.map(t =>
    `<div class="harness-target-chip" style="font-size:11px;padding:1px 6px">
      <span>${esc(t.label)}</span>
      <span class="remove-chip" onclick="harnessOrchRemoveTarget('${t.key.replace(/'/g,"\\'")}')">✕</span>
    </div>`
  ...
  function getOrCreateWorkerPane(round, target, prompt, rationale) {
    ...
    el.innerHTML = `
      <div class="harness-pane-header">
        ...
        <span class="harness-status pending" id="orch-status-${paneKey}">⟳ running…</span>
      </div>
      ...
      <div class="harness-pane-output thinking" id="orch-output-${paneKey}">waiting…</div>`;
...
async function loadConfig() {
  ...
  try {
    ...
    html += kv('model', esc(c.decision_llm?.model || '—'));
    ...
    html += kv('model', esc(c.operator?.model || '—'));
    ...
    // Individual tool toggles (informational — toggling requires config.yaml edit for now)
    ...
    // Flight recorder
    html += cfgToggle('flight_recorder_enabled', 'Flight recorder', c.flight_recorder?.enabled ?? false);
    html += `<div id="sub-flight_recorder_enabled" style="display:${c.flight_recorder?.enabled ? 'block' : 'none'};padding:4px 0 4px 24px;border-left:2px solid var(--bg-border);margin:2px 0 6px">`;
    html += cfgInput('flight_retention_hours', 'Retention (hours)', c.flight_recorder?.retention_hours ?? 24, 'number');
    html += cfgInput('flight_max_records',     'Max records',       c.flight_recorder?.max_records ?? 1000, 'number');
    html += '</div>';
    ...
    // Semantic map
    html += cfgToggle('semantic_map_enabled', 'Semantic map', c.semantic_map?.enabled ?? false);
    html += `<div id="sub-semantic_map_enabled" style="display:${c.semantic_map?.enabled ? 'block' : 'none'};padding:4px 0 4px 24px;border-left:2px solid var(--bg-border);margin:2px 0 6px">`;
    html += cfgInput('semantic_similarity_threshold', 'Similarity threshold (0-1)', c.semantic_map?.similarity_threshold ?? 0.5, 'number');
    html += cfgInput('semantic_max_topics',           'Max topics',                c.semantic_map?.max_topics ?? 50, 'number');
    html += '</div>';
    ...
    html += kv('prefix', esc(c.model_advertising?.prefix || '—'));
    ...
    html += cfgToggle('voice_enabled',  'Voice / PTT (requires STT service)', c.web_ui?.voice_enabled ?? false);
    html += cfgInput('voice_hotkey',    'Voice hotkey', c.web_ui?.voice_hotkey || '');
    ...
    ['cost_tracking_enabled','orchestrator_enabled','flight_recorder_enabled',
     'semantic_map_enabled','auto_summarization_enabled'].forEach(id => {
      const cb = document.getElementById(`cfg-${id}`);
      const sub = document.getElementById(`sub-${id}`);
      if (cb && sub) {
        cb.addEventListener('change', () => { sub.style.display = cb.checked ? 'block' : 'none'; });
      }
    });
...
async function saveConfig() {
  ...
  const numbers = [
    ...
    'flight_retention_hours','flight_max_records',
    'semantic_similarity_threshold','semantic_max_topics',
  ...
  try {
    ...
    if (status) status.textContent = `✓ saved (${d.saved?.length || 0} keys)`;
