# AI Middleware - Local LLM Conversation Proxy
...
### 1. FastAPI Proxy (middleware/main.py, middleware/proxy.py)
...
### 2. SQLite Storage (middleware/storage/sqlite_store.py)
...
### 3. Vector Storage (middleware/storage/vector_store.py)
...
### 4. LangChain Tools (middleware/tools/)
...
### 5. Decision LLM (Future - middleware/agents/decision.py)
...
```bash
# Clone the repo
git clone https://github.com/YOUR_USERNAME/ai-middleware.git
cd ai-middleware
...
1. Create a new file in `middleware/tools/` (e.g., `arxiv_search.py`)
2. Implement the tool following the pattern in existing tools
3. Register it in `middleware/tools/registry.py`
