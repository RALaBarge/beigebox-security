...
beigebox/
  ...
  beigebox/
    ...
    web/
      index.html                 single-file web UI, 7 tabs, no build step
...
```bash
cd docker
cp env.example .env          # edit OLLAMA_DATA to point at your ~/.ollama
docker compose up -d         # full stack: Ollama + BeigeBox + Open WebUI
```

That's it. On first start it pulls `llama3.2:3b` and `nomic-embed-text` automatically.

### Access

| Service    | URL                      | Notes                                    |
|------------|--------------------------|------------------------------------------|
| BeigeBox   | http://localhost:1337    | Web UI (port 1337 → internal 8000)       |
| Open WebUI | http://localhost:3000    | Chat frontend, pre-pointed at BeigeBox   |
| Ollama     | http://localhost:11434   | Direct inference if you need it          |

Open WebUI is already configured to talk to BeigeBox. Just open it and start chatting.

> **Pointing Open WebUI at BeigeBox manually?** Use `http://beigebox:8000/v1` (container-to-container) or `http://localhost:1337/v1` (from your host) with any non-empty API key.

### Add voice (optional)

```bash
docker compose --profile voice up -d   # adds Whisper STT + Kokoro TTS
```
...
Enable voice in BeigeBox via the Config tab (toggle **Voice / PTT**).

### Already have Ollama running?

Point BeigeBox at it instead of running a second instance:

```bash
# in docker/.env
OLLAMA_HOST=192.168.1.x        # LAN IP, or host.docker.internal for same machine

docker compose up -d beigebox open-webui   # skips the ollama container
```

### Dev loop

```bash
docker compose up -d --build beigebox   # rebuild BeigeBox only, leaves Ollama/WebUI up
...
# Core suite, no external dependencies needed
...
214 tests, all passing. The full suite uses only the packages in `requirements.txt` and
does not require Ollama or any running services.
...
- [x] Single-file web UI -- 7 tabs, full config editor, persistent state
...
See [`2600/CHANGELOG.md`](2600/CHANGELOG.md) for the full version history.
