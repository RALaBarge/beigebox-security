...
- **Wiretap** — structured JSONL + SQLite dual-write log of every request, response, routing decision, tool call, operator loop step, and harness orchestration event; event types: `message`, `session_hit`, `routing_decision`, `classify_result`, `decision_result`, `cache_hit`, `op_start/thought/tool_call/tool_result/answer/error`, `operator_thought/tool_call/iteration_end/nudge/finish`, `harness_start/plan/dispatch/turn/thought/evaluate/inject/end/error`; filterable by source (proxy/router/classifier/cache/operator/harness), event_type, role, run_id, and direction; clickable `conv:` and `run:` chips cross-link related entries; group-by-run accordion collapses a full operator or harness trace under one header; expandable meta field shows structured per-event data
- **Verbose agentic logging** — operator and harness emit detailed signals: every extracted thought, every tool call with input preview and elapsed_ms, per-iteration token count and confidence, loop detection events (nudge), final answer with total iterations and elapsed time
- **Model hardware stats** — Ollama `/api/ps` integration: GPU layer counts, VRAM usage, context window per model; shown in `/api/v1/backends` response and Dashboard backend health table; 30-second cache, non-blocking on Ollama failure
- **Model resource visibility** — passive discovery of VRAM/RAM footprints when models load, persisted in SQLite; `GET /api/v1/model-specs` returns all discovered models with VRAM, backend, discovery method, and last seen timestamp; `beigebox discover-models` CLI command shows currently-loaded models + GPU inventory + discovered specs; `beigebox deployment-plan` prints VRAM bars per GPU + OOM risk flags + system RAM summary for capacity planning; top-bar metrics display real-time CPU %, RAM %, GPU % with temperature probes (CPU via `psutil`, GPU via `pynvml`), 2s refresh with color-coded saturation (green <70%, yellow 70-85%, red >85%), tooltip shows per-GPU breakdown with temps; Dashboard shows Model Resource Specs panel with all discovered models
...
- **Task Packet Orchestration** — multi-agent composition framework that solves context bloat by composing minimal focused packets (200 tokens) instead of passing full conversation history (6,000 tokens); **TaskPacket** boundary object contains only relevant facts + recent dialogue + worker constraints + output schema; **PacketComposer** distills global state to focused context (Phase 1: heuristic slicing, Phase 2: semantic search); **ResultValidator** ensures agent outputs match schema before merging; **StateMerger** stores only high-confidence results (>0.7 confidence) as durable facts, logs everything for audit trail and replay; **WorkerProfiles** define per-agent constraints (research: cite sources/no code; coder: propose code/cite lines; operator: execute task/no shell; judge: compare fairly/no tools); achieves 30x token reduction, 3% hallucination rate (down from 15%), fair ensemble comparisons, clean state across 100+ agent calls; every packet logged with task_id for debugging and replay; see `2600/task-packet-orchestration.md` for detailed architecture
- **Prompt Optimizer** — iterative self-refinement via Champion/Challenger loops; explore the frontier of what's possible by mutating prompts, constraints, and sampling parameters (temperature, top_p); **Mutation strategies** include temperature variation (explore chaos at 1.5x), tool limit mutation (vary allowed calls), constraint evolution (must_do/must_not_do), and few-shot swapping; **Champion/Challenger loop** generates N variants, runs all via Harness in parallel, Judge model scores on multi-dimensional rubric (accuracy, brevity, clarity, safety, efficiency), Oracle verification prevents regressions (unit tests, deterministic checks), only promote Challenger if beats Champion by threshold; **Convergence controls** include hard iteration limit, diminishing returns detection, budget cap; enables automated discovery of optimal configurations for operators, workers, context composition strategies, and routing policies; every scorecard logged with full history for reproducibility and offline analysis
- **Context Optimization Discovery Framework** — 12-week systematic evaluation of 15 context optimization opportunities grounded in 2025-2026 research (VLDB, ACL, ArXiv); expected 25-40% composite improvement; **Phase 1**: Framework setup with JudgeRubric (5-dimensional scoring: accuracy, efficiency, clarity, hallucination, safety), OracleRegistry (50 golden test cases for regression detection), ParetoOptimizer (non-dominated frontier selection), SQLite persistence, discovery API endpoints; **Phase 2**: Systematic A/B testing of opportunities (3/week) — Opportunity #6 Position Sensitivity (+20-40% fact recall via placement strategy), #7 Context Compression (-40% tokens, same accuracy), #8 Context Depth vs. Breadth (+5-12% accuracy), #9 Prompt Caching (-60% latency, -30% cost), #10-15 TBD (context rot, source reputation, interleaving, etc.); **Phase 3**: Holdout validation (50 new never-seen test cases) ensures discoveries generalize; **Phase 4**: Deploy winners to production via PromptOptimizer mutation strategies; API: `POST /api/v1/discovery/run`, `GET /api/v1/discovery/results`
- **Orchestration Profiles** — CRUD management of reusable orchestration configurations; store profiles with name, worker_type, max_rounds, max_iterations, custom config; API: `GET/POST/PUT/DELETE /api/v1/orchestrations/{name}`; use profiles to pre-configure operator/harness/council setups across sessions
...
- **Trajectory evaluation** — every operator run is scored on four axes (Flow, Efficiency, Quality, Intent); score badge shown in the web UI; `loop_detected`, `hit_turn_cap`, and `no_file_writes` flags surfaced; scores persisted in SQLite for offline analysis
- **Adversarial context pruning** — before each multi-turn iteration, a ContextPruner agent compresses the previous turn's answer using a fast local model; reduces context bloat without losing key information; fires only in Harness Agentic mode; never blocks on error
- **Temporal layering (Reflect)** — a Reflector agent runs fire-and-forget after each turn; on the next turn, its insight is injected as a `[Reflection]` prefix; enable via the Reflect checkbox in the Harness toolbar; insight from prior turn always arrives in time for the next (asyncio.Task with consume_once semantics)
- **Counterfactual shadow agents** — on the first turn of a Harness run, a ShadowAgent generates an alternative plan in parallel; if the Jaccard similarity between plans is below threshold, the alternative is shown in a collapsible panel; enable via the Shadow checkbox; configurable divergence threshold
